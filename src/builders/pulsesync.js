...
const {
  ensureDir, copyRecursive, findFiles,
  minifyAndWrite, applyReplacementsToFile,
  createZip, resolveArtifactName, themeFolderName,
  IMAGE_EXTS, fileSize,
} = require('../utils');
...
function buildPulseSync(config) {
  ...
  if (zipConfig) {
    const zipName = resolveArtifactName(zipConfig.artifactName, config, 'pulsesync');
  ...
  if (pextConfig) {
    const pextName = resolveArtifactName(pextConfig.artifactName, config, 'pulsesync');
    const pextPath = path.join(cwd, 'dist', pextName);
    createZip(pextPath, fs.readdirSync(outDir).map(e => ({
      disk: path.join(outDir, e), archive: e,
    })));
