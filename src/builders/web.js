...
function buildWebReplacementMap(config) {
    ...
    for (const item of replacements) {
        ...
        const to   = item?.to;
        if (!from) continue;
        if (to) { map.push({ from, to }); continue; }

        const fileName = extractFileNameFromUrlOrPath(from);
        if (!fileName) { log.warn(`web replace skipped: cannot resolve filename from ${from}`); continue; }

        const assetFile = findAssetFile(assetsDir, fileName);
        if (!assetFile) { log.warn(`web replace skipped: asset not found — ${fileName}`); continue; }

        map.push({ from, to: fileToDataUrl(assetFile) });
        log.file('inline', `${from} → assets/${path.relative(assetsDir, assetFile)}`);
    }
...
function buildWebOnefile(config) {
    ...
    const artifactName = resolveArtifactName(onefileCfg.artifactName, config, 'web');
...
export function buildWeb(config) {
    log.task('web');
    log.info('building', { target: 'web', themeName: config.themeName, version: config.version });
