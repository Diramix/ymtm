...
const IMAGE_EXTS = ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.avif', '.svg', '.ico', '.bmp', '.tiff'];
...
function createZip(outputPath, entries) {
  ...
  for (const file of files) {
    ...
    const method = (file.disk === null || compressed.length === (file.disk ? fs.statSync(file.disk).size : 0))
      ? 0   // stored
      : 8;  // deflated
    ...
    const finalData = isDir ? Buffer.alloc(0) : (useDeflate ? compressed : fs.readFileSync(file.disk));
