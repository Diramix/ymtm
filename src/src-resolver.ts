...
function findDirsNamed(root: string, name: string, ignoreRules: string[] = []): string[] {
    const found: string[] = [];
...
function collectShared(dir: string, result: string[], ignoreRules: string[], skipPrefixes: Set<string>): void {
...
function collectAll(dir: string, result: string[], ignoreRules: string[]): void {
    if (!fs.existsSync(dir)) return;
    const entries = fs
        .readdirSync(dir, { withFileTypes: true })
        .sort((a, b) => a.name.localeCompare(b.name));
...
export function copyAssetsToOut(srcDir: string, outDir: string, ignoreRules: string[] = []): void {
    const assetsDirs = findAssetsDirs(srcDir, ignoreRules);
...
export function getBundleNames(metadata: Metadata | null): { js: string; css: string } {
    return {
...
export function relativeOutputPath(filePath: string, srcDir: string, targetFolder: string): string {
